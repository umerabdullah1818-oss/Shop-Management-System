...
/**
 * REP-009: the same report shapes as apps/local-server's ReportsService,
 * against the cloud replica — necessarily lags behind live shop data by
 * however long sync takes (Decision #11's read-mostly remote access).
 * `asOf` + `cloudLastSyncedAt` (from Settings' "lastIngestAt" heartbeat,
 * written by SyncService.ingest) tell the caller how stale this view is.
 */
@Injectable()
export class ReportsService {
  ...
  // Strict FIFO COGS, never an average (BR-022) — identical to local's calculation.
  ...
  async dashboard() {
    ...
    const [todaySales, lowStock, khata, suppliers, profit, recentSales, lastIngest] = await Promise.all([
      ...
      this.prisma.setting.findUnique({ where: { key: "lastIngestAt" } }),
    ]);
    ...
    return {
      ...
      cloudLastSyncedAt: (lastIngest?.value as string | undefined) ?? null,
    };
  }
}
